...
// Use that random index in allWords and set that to a new variable that will hold our selectedWord.
...
function check() {
  if (selectedWord === document.getElementById("myWord").value.trim().toLowerCase()) {
    alert("CORRECT!");
    unscrambles++;
  document.getElementById("unscrambled").innerHTML=document.getElementById("unscrambled").innerHTML + "<br>" + selectedWord;
 nextWord();
  }else{
    gameDone();
  }
...
function gameDone(){
  isGameOver = true;
  alert("Game Over! You unscrabled " + unscrambles + " word/words, please refresh your screen.");
}
